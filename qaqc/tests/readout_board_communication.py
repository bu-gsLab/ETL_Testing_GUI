...
from qaqc.errors import FatalTestError
...
def test(session):
    ...
    """
    try:
        session.report_status("Connecting to the KCU...")
        ...
        session.report_status("Connecting to the readout board...")
        ...
        session.report_status("Checking master LPGBT communication...")
        ...
        if wr_temp != wr_temp_check:
            raise FatalTestError(f"Master LPGBT communication failed. Attempted write of {wr_temp} but read back {wr_temp_check}")
        ...
        session.report_status("Checking servant LPGBT communication...")
        ...
        if wr_temp != wr_temp_check:
            raise FatalTestError(f"Servant LPGBT communication failed. Attempted write of {wr_temp} but read back {wr_temp_check}")
        rb.TRIG_LPGBT.wr_reg("LPGBT.RWF.CHIPID.USERID0", val)

    except FatalTestError:
        raise
    except Exception as e:
        raise FatalTestError(str(e)) from e
